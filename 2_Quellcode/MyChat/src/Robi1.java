...
public class Robi1 extends Thread {
	...
	Robi1(Grundstueck1 gs, double maxV, double winkel, int posX, int posY, Color f, int b, int h) {
		doMove = false;
		grund = gs;
		maxStep = maxV;
		richtung = winkel;
	...
	void fahren(double speedL, double speedR) {
		...
		if ( spd < 1e-2)
			return;
        richtung = richtung+0.1*(Math.atan2(speedL,speedR)-Math.PI/4);
        position.x += maxStep*spd*Math.cos(richtung);
        position.y += maxStep*spd*Math.sin(richtung);
	}
	
	Polygon ausrichten() {
		// Drehen um den den Mittelpunkt um den Winkel richtung
